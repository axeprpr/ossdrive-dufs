...
impl App {
    async fn call(self: Arc<Self>, req: Request<Incoming>, addr: Option<SocketAddr>) -> Result<server::Response, hyper::Error> {
        match self.as_ref() {
            Self::Local(server) => server.clone().call(req, addr).await,
            Self::Oss(server) => server.clone().call(req, addr).await,
        }
    }
}
...
async fn handle_stream<T>(handle: Arc<App>, stream: TokioIo<T>, addr: Option<SocketAddr>)
where
