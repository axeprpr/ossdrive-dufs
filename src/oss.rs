...
impl OssServer {
    ...
    fn index_page(&self) -> Response {
        ...
        let mut html = include_str!("../assets/index.html").replace("__ASSETS_PREFIX__", &assets_prefix).replace("window.__DUFS_PREFIX__=\"/\"", &format!("window.__DUFS_PREFIX__=\"{}\"", assets_prefix));
